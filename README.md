...
1. **配置 LLM**（AI 建图谱必需）：设置页填 DeepSeek API Key + 测试连接；模型推荐 `deepseek-chat`（非推理，快且稳；推理模型如 `deepseek-reasoner` 建图谱会很慢）
2. **准备教材 PDF**：打开检索页直接拖拽上传 PDF（推荐）；也可手动放入 `~/.huoshu/pdf/`（Windows: `C:\Users\<你>\.huoshu\pdf\`）
3. **生成图谱**：AI 建图谱页 → 选 PDF → 选目标 → 点生成（实时进度可见，生成前会预估调用次数）→ 校对 → 保存（图谱页/学习页/复习页立即可用）
4. **Ollama 就绪**（教材检索需要）：启动 Ollama 后运行 `ollama pull nomic-embed-text`

### 学习时补充图谱

基础图谱建好后，学习过程中遇到新概念：学习会话页 → 点「➕ 补充新知识点」→ 输入名称与描述、选择与当前知识点的关系 → AI 自动抽取结构化字段并加入图谱，保存后图谱页立即可见。图谱随学习持续完善，无需重新建整本。
